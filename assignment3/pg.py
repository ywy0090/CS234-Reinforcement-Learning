...
def build_mlp(
    ...
    if n_layers <= 0:
        ...
        for i in range(n_layers):
            out = tf.layers.dense(out, size, activation=output_activation, )
...
class PG(object):
    ...
    def add_placeholders_op(self):
        ...
        self.observation_placeholder = tf.placeholder(tf.float32, [None, self.observation_dim], name='observation') # TODO
        if self.discrete:
            self.action_placeholder =  tf.placeholder(tf.int64, None, name='action')# TODO
        else:
            self.action_placeholder =  tf.placeholder(tf.float32, [None, self.action_dim], name='action')# TODO

        # Define a placeholder for advantages
        self.advantage_placeholder =  tf.placeholder(tf.float32, shape=(), name='advantage')# TODO
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def build_policy_network_op(self, scope="policy_network"):
        ...
        if self.discrete:
            action_logits = build_mlp(mlp_input=self.observation_placeholder, output_size=self.action_dim,
                                      scope=scope,
            ...
            action_means = build_mlp(mlp_input=self.observation_placeholder, output_size=self.action_dim,
                                     scope=scope,
                                     n_layers=self.config.n_layers,
                                     size=self.config.layer_size) # TODO
            log_std = tf.math.log(tf.get_variable('sigma')) # TODO
            self.sampled_action = tf.random_normal([self.action_dim], action_means, tf.exp(log_std)) # TODO
            self.logprob = tf.contrib.distributions.MultivariateNormalDiag(loc=action_means, scale_diag=tf.exp(log_std))\
                .log_prob(self.action_placeholder)  # TODO
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def add_baseline_op(self, scope="baseline"):
        ...
        self.baseline = tf.squeeze(build_mlp(mlp_input=self.observation_placeholder, output_size=self.action_dim,
                                      scope=scope,
                                      n_layers=self.config.n_layers,
                                      size=self.config.layer_size)) # TODO
        ...
        self.update_baseline_op = tf.train.AdamOptimizer(self.lr).minimize(self.baseline_loss)  # TODO
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            rewards_sum = 0
            for r in rewards:
                rewards_sum += tmp_rate * r
                tmp_rate = self.config.gamma * tmp_rate
            returns = rewards_sum # TODO
            #######################################################
            #########          END YOUR CODE.          ############
    ...
    def calculate_advantage(self, returns, observations):
        ...
        adv = returns.copy()
        if self.config.use_baseline:
            baseline_n = self.sess.run(self.baseline)
            adv = returns - baseline_n
        if self.config.normalize_advantage:
            baseline_n = self.sess.run(self.baseline)
            adv = returns - baseline_n
            mean, variance = tf.nn.moments(adv, axes=[0,1])
            adv = (adv - mean) / tf.sqrt(variance)
            # TODO
        #######################################################
        #########          END YOUR CODE.          ############
        return adv
